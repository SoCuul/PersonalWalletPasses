...
const [ configFilePath, openAfterCompletion ]: string[] = process.argv.splice(2)
...
if (passConfig.source.length && passConfig.out.length) {
    const imageFiles = [ 'icon', 'logo', 'strip', 'thumbnail', 'background' ] as const

    // Prepare requested images
    for (const image of imageFiles) {
        if (passConfig.assets[image]) await fetchImage(
            resolvePath(passConfig.source, `assets/${image}.png`), `${image}@3x.png`
        )
    }
}
else {
...
// Load certificates
const wwdr: Buffer = readFileSync(resolvePath(passConfig.source, 'certs/wwdc/AppleWWDRCAG4.pem'))
const signerCert: Buffer = readFileSync(resolvePath(passConfig.source, 'certs/signer/signerCert.pem'))
const signerKey: Buffer = readFileSync(resolvePath(passConfig.source, 'certs/signer/signerKey.pem'))
const signerKeyPassphrase: string = readFileSync(resolvePath(passConfig.source, 'certs/signer/passphrase.txt'), 'utf8')

// Create pass base
const pass: passkit.PKPass = await PKPass.from({
